...
bool restar_y_detectar_zeroflag(int a, int b, int *resultado) {
    ...
    __asm__ (
        ...
        : [val_b] "r" (b)               // Entrada: 'b'
        : "cc"                          // Informa al compilador que cambiamos las banderas
    );
...
int main() {
    ...
    bool es_cero = restar_y_detectar_zeroflag(42, 42, &res);
    printf("42 - 42 = %d | Zero Flag activa: %s\n", res, es_cero ? "SI" : "NO");

    // Caso 2: Los valores son distintos (no activa ZF)
    es_cero = restar_y_detectar_zeroflag(50, 20, &res);
